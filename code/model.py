...
class RobertaMoEForSequenceClassification(nn.Module):
    def __init__(self, config,num_public_layers=12, num_experts=8 ,num_labels=3, num_gate_layer=2):
    ...
    def forward(
        ...
        final_out = []
        for i in range(self.num_experts):
            final_out.append(self.classifiers[i](outputs[i][:,0,:]).unsqueeze(1)) # bsz * 1 * 3
        final_out_mat = torch.nn.functional.softmax(torch.cat(final_out, dim=1), dim=2) # bsz * num of exp * 3
