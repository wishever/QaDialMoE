...
class DataProcessor(object):
    def get_examples(self, data_dir, dataset=None):
        ...
        data_dirs = data_dir.split(',') if ',' in data_dir else [data_dir]
        lines = []
        for data in data_dirs:
            lines += self._get_json_lines(os.path.join(data, "{}.jsonl".format(dataset)))
    ...
    def _create_examples(self, lines, max_evidences=3):
        examples = []
        obj = TfIdfSimilarity()
        for i, datapoint in enumerate(tqdm(lines)):
            if 'dpr_evidence' in datapoint.keys():
                ...
                all_evidences = datapoint['dpr_evidence'][:max_evidences]
                all_evidence_texts = ['title: ' + x['title'] + ' content: ' + x['passage'] for x in all_evidences]
...
def run_eval(device, processor, tokenizer, model, writer, global_step, tensorboard=False,
    ...
    json.dump(mapping, open('./{}_moe_roberta_lmd_dpr.json'.format(phase),'w', encoding='utf8'))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", help="path to data", default='../data/faviq/dpr/faviq_r_set')# ,../data/faviq/dpr/faviq_r_set
    parser.add_argument("--train_set", default="final_train")
    parser.add_argument("--dev_set", default="final_dev")
    parser.add_argument("--test_set", default="final_test")
    parser.add_argument("--output_dir", default='./outputs_faviqmoe_dpr')
    ...
    parser.add_argument('--wait_step', type=int, default=20)
    ...
    # TRANSFORMERS_OFFILNE=1 python run_faviqmoe_dpr.py --do_train --do_eval
    # TRANSFORMERS_OFFILNE=1 CUDA_VISIBLE_DEVICES=0 nohup python -u run_faviqmoe_dpr.py --do_train --do_eval --load_dir outputs_faviqmoe_dpr/saved_model > faviq_log_dpr.file 2>&1 &
    # python run_dialmoe.py --do_eval --do_test --load_dir [modeldir]